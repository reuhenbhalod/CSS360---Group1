...
function buildFeedItems(data, feedFilter) {
  ...
  if (feedFilter === "all" || feedFilter === "news") {
    (data.news?.data || []).forEach((a) =>
      items.push({ kind: "gnews", data: a, sort: new Date(a.publishedAt).getTime() })
    );
    (data.articles?.data || []).forEach((a) =>
      items.push({ kind: "guardian", data: a, sort: new Date(a.publishedAt).getTime() })
    );
  }
...
describe("buildFeedItems", () => {
  ...
  it("handles empty data sources gracefully", () => {
    const emptyData = { reddit: { data: [] }, news: { data: [] }, articles: { data: [] } };
...
describe("buildMapPins", () => {
  ...
  it("filters out pins with non-finite lat/lon", () => {
    const badData = {
      restaurants: {
        data: [
          { id: "bad", name: "No Coords", lat: NaN, lon: -122.2, source: "foursquare" },
          MOCK_RESTAURANT_1,
        ],
      },
      places: { data: [] },
    ...
    expect(pins.every((p) => Number.isFinite(p.lat) && Number.isFinite(p.lon))).toBe(true);
    expect(pins).toHaveLength(1);
  });

  it("returns empty array when data has no restaurants or places", () => {
    expect(buildMapPins({ restaurants: { data: [] }, places: { data: [] } })).toHaveLength(0);
  });
